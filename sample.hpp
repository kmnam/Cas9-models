...
boost::random::gamma_distribution<> DIST(1.0);

MatrixXd sampleFromSimplex(const MatrixXd& vertices, unsigned npoints, boost::random::mt19937& rng)
{
    /*
    ...
    for (unsigned i = 0; i < npoints; i++)
    {
        ...
        for (unsigned j = 0; j < dim + 1; j++) barycentric(i,j) = DIST(rng);
