...
MatrixX200 computeRejectionStats(const Ref<const VectorX200>& params)
{
    ...
    std::array<mpfr_200_noet, 6> match_params;
    ...
    std::array<mpfr_200_noet, 6> mismatch_params;
    ...
    GridGraph<mpfr_200_noet>* model = new GridGraph<mpfr_200_noet>(length);
    ...
    // to cleaved state
    Matrix<mpfr_200_noet, 2, 1> match_data = model->computeRejectionStats(1, 1).array().log10().matrix();
    ...
    for (unsigned j = 1; j <= length; ++j)
    {
        ...
        Matrix<mpfr_200_noet, 2, 1> mismatch_data = model->computeRejectionStats(1, 1).array().log10().matrix();
