 ...
 * - cleavage specificity with respect to the distal-mismatch substrate
...
std::function<VectorXd(const Ref<const VectorXd>&)> getCleavageFunc(int position, int dim)
{
    ...
    else if (dim == 5)
    {
        switch (position)
        {
            ...
            case 20:
                return computeCleavageStatsUnbindingUnity<PreciseType, 20>;
            default:
                throw std::invalid_argument("Invalid number of mismatches");
        } 
    }
    else if (dim == 4)
    {
        switch (position)
        {
            ...
            case 20:
                return computeCleavageStatsBothTerminalUnity<PreciseType, 20>;
            default:
                throw std::invalid_argument("Invalid number of mismatches");
        } 
    }
...
int main(int argc, char** argv)
{
    ...
    std::function<VectorXd(const Ref<const VectorXd>&)> func = getCleavageFunc<PreciseType>(num_mismatches, finder->getD());
    finder->setFunc(func); 
