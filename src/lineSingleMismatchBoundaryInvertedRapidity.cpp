 ...
 * subregion with inverted (negative) rapidity for the line graph.
 *
...
VectorXd computeCleavageStats(const Ref<const VectorXd>& input) 
{
    ...
    PreciseType rate_perfect = model->getUpperExitRate(terminal_unbind_rate, terminal_cleave_rate); 
    ...
    PreciseType rate_mismatched = model->getUpperExitRate(terminal_unbind_rate, terminal_cleave_rate);  
...
{
    // Define filtering function that restricts output to >= 0.01 in 
    // absolute value 
