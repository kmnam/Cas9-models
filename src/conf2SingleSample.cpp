 ...
 * the two-conformation Cas9 model (grid graph) against single-mismatch
 * substrates.  
 *
 * Authors:
...
int main(int argc, char** argv)
{
    ...
    // Compute efficiencies and mean first passage times 
    ...
    if (specfile.is_open())
    {
        for (unsigned i = 0; i < probs.rows(); i++)
        {
            ...
            specfile << probs(i,probs.cols()-1) << std::endl;
        }
    }
    specfile.close();
    oss.clear();
