 ...
 * - PSLG: position-specific line graph
...
/**
 * Generate the given number of R-loop PSLGP vectors that satisfy the
 * fundamental constraints (b >= b' and d <= d'). 
 *
 * @param bounds  Bounds for b, d, b', d' (assumed to be uniform over
 *                position).
 * @param nsample Number of PSLGP vectors to be sampled.
 * @param rng     Random number generator instance.
 */
Matrix<MainType, Dynamic, Dynamic> samplePositionSpecificParams(const Ref<const Matrix<mpq_rational, Dynamic, 2> >& bounds,
                                                                const int nsample,
                                                                boost::random::mt19937& rng)
{
    // Check that the bounds are valid
    if (bounds(0, 1) < bounds(2, 1))    // max(b) should be >= max(b')
        throw std::invalid_argument("Invalid parameter bounds specified"); 
    if (bounds(1, 0) > bounds(3, 0))    // min(d) should be <= min(d')
        throw std::invalid_argument("Invalid parameter bounds specified");

    Matrix<MainType, Dynamic, Dynamic> sample(nsample, 2 + 2 * length);
    MainType b_max = static_cast<MainType>(bounds(0, 1));
    MainType d_min = static_cast<MainType>(bounds(1, 0)); 
    MainType bp_min = static_cast<MainType>(bounds(2, 0));
    MainType bp_diff = static_cast<MainType>(bounds(2, 1) - bounds(2, 0)); 
    MainType dp_min = static_cast<MainType>(bounds(3, 0));
    MainType dp_diff = static_cast<MainType>(bounds(3, 1) - bounds(3, 0)); 
    for (int i = 0; i < nsample; ++i)
    {
        // Generate the mismatch parameters first
        Matrix<MainType, Dynamic, 1> bp(length); 
        Matrix<MainType, Dynamic, 1> dp(length);
        boost::random::uniform_01<double> dist;
        for (int j = 0; j < length; ++j)
        {
            bp(j) = bp_min + bp_diff * static_cast<MainType>(dist(rng));
            dp(j) = dp_min + dp_diff * static_cast<MainType>(dist(rng));
        }

        // Then generate the match parameters, conditioned on the sampled mismatch
        // parameter values
        MainType bp_sample_max = bp.maxCoeff(); 
        MainType dp_sample_min = dp.minCoeff();  
        sample(i, 0) = bp_sample_max + (b_max - bp_sample_max) * static_cast<MainType>(dist(rng)); 
        sample(i, 1) = d_min + (dp_sample_min - d_min) * static_cast<MainType>(dist(rng));
        sample(i, Eigen::seqN(2, length)) = bp; 
        sample(i, Eigen::seqN(2 + length, length)) = dp;
    }
 ...
 * specified in the given matrix of complementarity patterns, for the given PSLG.
 *
 * Here, `logrates` is assumed to contain *43* entries:
 * 1) single forward rate at DNA-RNA matches,
 * 2) single reverse rate at DNA-RNA matches,
 * 3) forward rate at DNA-RNA mismatches at each position,
 * 4) reverse rate at DNA-RNA mismatches at each position,
 * 5) single terminal unbinding rate.
 *
 * @param logrates  Input vector of 43 PSLGPs.
                                                   ...
                                                   const Ref<const MatrixXi>& seqs)
{
    ...
    PreciseType terminal_unbind_rate = pow(ten_precise, static_cast<PreciseType>(logrates(2 * length + 2)));
    ...
    for (int i = 0; i < seqs.rows(); ++i)
    {
        for (int j = 0; j < length; ++j)
        {
            std::pair<PreciseType, PreciseType> mismatch_rates = std::make_pair(
                pow(ten_precise, static_cast<PreciseType>(logrates(2 * (j + 1)))),
                pow(ten_precise, static_cast<PreciseType>(logrates(2 * (j + 1) + 1)))
            ); 
            if (seqs(i, j))
 ...
 * the given PSLG.
 *
 * Here, `logrates` is assumed to contain *45* entries:
 * 1) single forward rate at DNA-RNA matches,
 * 2) single reverse rate at DNA-RNA matches,
 * 3) forward rate at DNA-RNA mismatches at each position,
 * 4) reverse rate at DNA-RNA mismatches at each position,
 * 5) single terminal unbinding rate,
 * 6) single terminal cleavage rate,
 * 7) single terminal binding rate (units of inverse (M * sec)).
 *
 * @param logrates  Input vector of 45 PSLGPs.
 * @param seqs      Matrix of input sequences, with entries of 0 (match w.r.t.
 *                  perfect-match sequence) or 1 (mismatch w.r.t. perfect-match
 *                  sequence).
                                                       ...
                                                       const MainType bind_conc) 
{
    ...
    PreciseType terminal_unbind_rate = pow(ten_precise, static_cast<PreciseType>(logrates(2 * length + 2)));
    PreciseType terminal_cleave_rate = pow(ten_precise, static_cast<PreciseType>(logrates(2 * length + 3))); 

    // Binding rate entering state 0
    PreciseType bind_rate = pow(ten_precise, static_cast<PreciseType>(logrates(2 * length + 4))) * static_cast<PreciseType>(bind_conc);
 ...
 * 4) reverse rate at DNA-RNA mismatches at each position,
 * 5) single terminal unbinding rate,
 * 6) single terminal cleavage rate, and
 * 7) single terminal binding rate (units of inverse (M * sec)).
 *
 * @param logrates  Input vector of 45 PSLGPs.
 * @param seqs      Matrix of input sequences, with entries of 0 (match w.r.t.
 ...
 *
 * @param logrates    Input vector of 43 LGPs.
 ...
 * function/class with which this function will be used.
 *
 * @param logrates    Input vector of 45 LGPs.
 ...
 * @param bounds                  Bounds for the R-loop rates, b, d, b', d'
 *                                (assumed to be uniform over position).
                                            ...
                                            const bool zoom_verbose) 
{
    ...
    // Sample a set of initial parameter points 
    Matrix<MainType, Dynamic, Dynamic> init_points = samplePositionSpecificParams(bounds, ninit, rng);
    ...
    Matrix<MainType, Dynamic, 1> regularize_bases = (bounds.col(0) + bounds.col(1)).cast<MainType>() / 2;
    ...
    std::function<MainType(const Ref<const Matrix<MainType, Dynamic, 1> >&)> func = 
        ...
        {
            Matrix<MainType, Dynamic, 1> y(2 * length + 3); 
            y.head(2 * length + 2) = x; 
            y(2 * length + 2) = 0;
    ...
    for (int i = 0; i < ninit; ++i)
    {
        ...
        Matrix<MainType, Dynamic, 1> y(2 * length + 3); 
        y.head(2 * length + 2) = best_fits.row(i); 
        y(2 * length + 2) = 0;
                                            ...
                                            const bool zoom_verbose) 
{
    ...
    while (nsample < ninit)
    {
        ...
        mpq_rational min = bounds(0, 0);
        mpq_rational max = bounds(0, 1);
        mpq_rational x = min + (max - min) * static_cast<mpq_rational>(dist(rng));
        Matrix<mpq_rational, Dynamic, 1> p = x * Matrix<mpq_rational, Dynamic, 1>::Ones(D);
        for (int j = 1; j < length + 1; ++j)
        {
            // Use the constraints for the j-th weight w.r.t the 0-th weight:
            // 
            // weight for b/d - weight for b'/d' >= fit_logrates(2*j) - fit_logrates(0)
            // weight for b'/d' - weight for b/d >= fit_logrates(1) - fit_logrates(2*j+1)
            //
            // means that 
            //
            // weight for b'/d' <= weight for b/d - fit_logrates(2*j) + fit_logrates(0)
            // weight for b'/d' >= weight for b/d + fit_logrates(1) - fit_logrates(2*j+1)
            //
            min = x + static_cast<mpq_rational>(fit_logrates(1)) - static_cast<mpq_rational>(fit_logrates(2 * j + 1));
            max = x - static_cast<mpq_rational>(fit_logrates(2 * j)) + static_cast<mpq_rational>(fit_logrates(0));
                                                 // min should be less than max
            p(j) = min + (max - min) * static_cast<mpq_rational>(dist(rng));
        }
        for (int j = length + 1; j < D; ++j)
        {
            min = bounds(j, 0); 
            max = bounds(j, 1);
            p(j) = min + (max - min) * static_cast<mpq_rational>(dist(rng));
    ...
    std::function<MainType(const Ref<const Matrix<MainType, Dynamic, 1> >&)> func = 
        ...
        ) -> MainType
        {
            ...
            Matrix<MainType, Dynamic, 1> y(2 * length + 5);
    ...
    for (int i = 0; i < ninit; ++i)
    {
        ...
        for (int j = 0; j < length + 1; ++j)
            y(Eigen::seqN(2 * j, 2)) = fit_logrates(Eigen::seqN(2 * j, 2)) + best_fits(i, j) * Matrix<MainType, Dynamic, 1>::Ones(2);
...
int main(int argc, char** argv)
{
     ...
     *  ------------------------------------------------------- */
    mpq_rational abs_bound = 4;
    
    // Define the left-hand matrix ...
    Matrix<mpq_rational, Dynamic, Dynamic> A = Matrix<mpq_rational, Dynamic, Dynamic>::Zero(6 * length + 4, 2 * length + 2);
    A(Eigen::seqN(0, length), 0)
        = Matrix<mpq_rational, Dynamic, 1>::Ones(length); 
    A(Eigen::seqN(0, length), Eigen::seqN(2, length))
        = -Matrix<mpq_rational, Dynamic, 1>::Identity(length, length); 
    A(Eigen::seqN(length, length), 1)
        = -Matrix<mpq_rational, Dynamic, 1>::Ones(length);
    A(Eigen::seqN(length, length), Eigen::seqN(length + 2, length))
        = Matrix<mpq_rational, Dynamic, Dynamic>::Identity(length, length);
    A(Eigen::seqN(2 * length, 2 * length + 2), Eigen::all)
        = Matrix<mpq_rational, Dynamic, Dynamic>::Identity(2 * length + 2, 2 * length + 2);
    A(Eigen::seqN(4 * length + 2, 2 * length + 2), Eigen::all)
        = -Matrix<mpq_rational, Dynamic, Dynamic>::Identity(2 * length + 2, 2 * length + 2);

    // ... and the right-hand vector 
    Matrix<mpq_rational, Dynamic, 1> b = Matrix<mpq_rational, Dynamic, 1>::Zero(6 * length + 4);
    b.tail(4 * length + 4) = -abs_bound * Matrix<mpq_rational, Dynamic, 1>::Ones(4 * length + 4);

    // Define LinearConstraints instance 
    Polytopes::LinearConstraints* constraints_1 = new Polytopes::LinearConstraints(
        Polytopes::InequalityType::GreaterThanOrEqualTo, A, b
    );

    // Define matrix of bounds for each parameter 
    Matrix<mpq_rational, Dynamic, 2> bounds(2 * length + 2, 2); 
    bounds.col(0) = -abs_bound * Matrix<mpq_rational, Dynamic, 1>::Ones(2 * length + 2);
    bounds.col(1) = abs_bound * Matrix<mpq_rational, Dynamic, 1>::Ones(2 * length + 2);
    ...
    results_dissoc = fitLineParamsAgainstMeasuredRatesDissoc(
        constraints_1, bounds, unbind_data, unbind_seqs, ninit, rng,
    ...
    for (int i = 0; i < length + 1; ++i)
    {
        param_bounds(i, 0) = static_cast<mpq_rational>(weight_min); 
        param_bounds(i, 1) = static_cast<mpq_rational>(weight_max);
    }
    param_bounds(length + 1, 0) = static_cast<mpq_rational>(terminal_unbind_lograte_min);
    param_bounds(length + 1, 1) = static_cast<mpq_rational>(terminal_unbind_lograte_max);
    param_bounds(length + 2, 0) = static_cast<mpq_rational>(terminal_cleave_lograte_min);
    param_bounds(length + 2, 1) = static_cast<mpq_rational>(terminal_cleave_lograte_max);
    param_bounds(length + 3, 0) = static_cast<mpq_rational>(terminal_bind_lograte_min);
    param_bounds(length + 3, 1) = static_cast<mpq_rational>(terminal_bind_lograte_max);
    Matrix<mpq_rational, Dynamic, Dynamic> A2 = Matrix<mpq_rational, Dynamic, Dynamic>::Zero(2 * D + 2 * length, D); 
    Matrix<mpq_rational, Dynamic, 1> b2(2 * D + 2 * length);
    for (int i = 0; i < 2 * D; ++i)
    {
        ...
        {
            A2(i, j) = 1;
            b2(i) = param_bounds(j, 0);
        }
        else 
        {
            A2(i, j) = -1;
            b2(i) = -param_bounds(j, 1);
        }
    }
    for (int i = 0; i < length; ++i)
    {
        A2(2 * D + 2 * i, 0) = 1;         // weight for b/d - weight for b'/d' >= fit_logrates(2) - fit_logrates(0)
        A2(2 * D + 2 * i, i + 1) = -1;
        A2(2 * D + 2 * i + 1, 0) = -1;    // weight for b'/d' - weight for b/d >= fit_logrates(1) - fit_logrates(3)
        A2(2 * D + 2 * i + 1, i + 1) = 1;
        b2(2 * D + 2 * i) = static_cast<mpq_rational>(fit_logrates(2 * i + 2)) - static_cast<mpq_rational>(fit_logrates(0));
        b2(2 * D + 2 * i + 1) = static_cast<mpq_rational>(fit_logrates(1)) - static_cast<mpq_rational>(fit_logrates(2 * i + 3));
    }
    Polytopes::LinearConstraints* constraints_2 = new Polytopes::LinearConstraints(
        Polytopes::InequalityType::GreaterThanOrEqualTo, A2, b2
    );
    ...
    std::cout << residuals_cleave << "\n--\n";
    std::cout << errors_cleave << std::endl;
