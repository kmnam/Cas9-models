...
MatrixXd computeRejectionStats(const Ref<const VectorXd>& params)
{
    /*
    ...
    // to cleaved state
    Matrix<double, 2, 1> match_data = model->computeRejectionStats(1, 1).array().log10().matrix();
    ...
    for (unsigned j = 1; j <= length; ++j)
    {
        ...
        Matrix<double, 2, 1> mismatch_data = model->computeRejectionStats(1, 1).array().log10().matrix();
        
        // Compute the specificity and speed ratio
        double specificity = mismatch_data(0) - match_data(0);
        double speed_ratio = match_data(1) - mismatch_data(1);
...
int main(int argc, char** argv)
{
    ...
    {
        MatrixXd stats = computeRejectionStats(params.row(i));
