 ...
 * that specificity is small (> 0.0001 and < 0.001).
 *
 * **Authors:**
...
{
    // Define filtering function that excludes all output points with x > 0.001 and x <= 0.0001
    std::function<bool(const Ref<const VectorXd>&)> filter
        = [](const Ref<const VectorXd>& x)
        {
            return (x(0) > 0.001 || x(0) <= 0.0001);
        };
