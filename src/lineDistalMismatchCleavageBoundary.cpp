...
/*
 * Estimates the boundary of the cleavage specificity vs. conditional 
 * cleavage rate region in the line Cas9 model.
 *
 * Authors:
...
VectorXd computeCleavageStats(const Ref<const VectorXd>& params)
{
    ...
    // Compute cleavage probability and conditional mean first passage time
    // to cleaved state without any mismatches present
    T prob_perfect = model->computeUpperExitProb(1, 1);
    T rate_perfect = model->computeUpperExitRate(1, 1);
    ...
    T rate = model->computeUpperExitRate(1, 1);
