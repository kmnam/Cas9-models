...
VectorX30 computeCleavageStats(const Ref<const VectorX30>& params)
{
    ...
    // to cleaved state
    Matrix<mpfr_30_noet, 2, 1> match_data = model->computeCleavageStats(1, 10).array().log10().matrix();
    ...
    Matrix<mpfr_30_noet, 2, 1> mismatch_data = model->computeCleavageStats(1, 10).array().log10().matrix();
