...
VectorX30 computeCleavageStats(const Ref<const VectorX30>& params)
{
    ...
    // to cleaved state
    Matrix<mpfr_30_noet, 2, 1> match_data = model->computeCleavageStats(1, 100).array().log10().matrix();
    ...
    Matrix<mpfr_30_noet, 2, 1> mismatch_data = model->computeCleavageStats(1, 100).array().log10().matrix();
...
int main(int argc, char** argv)
{
    ...
    std::function<bool(const Ref<const VectorX30>& x)> filter = [](const Ref<const VectorX30>& x){ return false; };
    ...
    BoundaryFinder<mpfr_30_noet> finder(6, tol, rng, A, b);
