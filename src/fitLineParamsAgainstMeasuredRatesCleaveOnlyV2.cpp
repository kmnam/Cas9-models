...
/**
 * Compute the overall cleavage rates on all matched/mismatched sequences
 ...
 * @returns Vector of overall cleavage rates. 
 */
Matrix<MainType, Dynamic, 1> computeOverallCleavageRates(const Ref<const Matrix<MainType, Dynamic, 1> >& logrates,
                                                         ...
                                                         const MainType bind_conc) 
{
    ...
    for (int i = 0; i < seqs.rows(); ++i)
    {
        ...
        stats(i) = pow(overall_cleave_time, -1); 
    }
