...
MatrixX200 computeRejectionStats(const Ref<const VectorX200>& params)
{
    /*
    ...
    std::array<mpfr_200_noet, 2> match_params;
    match_params[0] = pow(10.0, params(0));
    match_params[1] = pow(10.0, params(1));

    // Array of DNA/RNA mismatch parameters
    std::array<mpfr_200_noet, 2> mismatch_params;
    mismatch_params[0] = pow(10.0, params(2));
    mismatch_params[1] = pow(10.0, params(3));

    // Populate each rung with DNA/RNA match parameters
    LineGraph<mpfr_200_noet>* model = new LineGraph<mpfr_200_noet>(length);
    for (unsigned j = 0; j < length; ++j)
        model->setLabels(j, match_params);
    
    // Compute cleavage probability and mean first passage time 
    // to cleaved state
    Matrix<mpfr_200_noet, 2, 1> match_data = model->computeRejectionStatsByInverse(1, 1).array().log10().matrix();

    // Introduce distal mismatches and re-compute cleavage probability
    // and mean first passage time
    MatrixX200 stats(length, 2);
    for (unsigned j = 1; j <= length; ++j)
    {
        model->setLabels(length - j, mismatch_params);
        Matrix<mpfr_200_noet, 2, 1> mismatch_data = model->computeRejectionStatsByInverse(1, 1).array().log10().matrix();
        
        // Compute the specificity and speed ratio
        mpfr_200_noet specificity = match_data(0) - mismatch_data(0);
        mpfr_200_noet speed_ratio = mismatch_data(1) - match_data(1);
...
int main(int argc, char** argv)
{
    ...
    try
    {
        std::tie(vertices, params) = sampleFromConvexPolytopeTriangulation<mpfr_200_noet>(argv[1], n, rng);
    }
    ...
    MatrixX200 specs(n, length);
    MatrixX200 speed(n, length);
    for (unsigned i = 0; i < n; ++i)
    {
        MatrixX200 stats = computeRejectionStats(params.row(i));
